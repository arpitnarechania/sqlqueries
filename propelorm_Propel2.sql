...
CREATE TABLE `Woopah`.`foo`( `id` INTEGER NOT NULL AUTO_INCREMENT, `bar` INTEGER, PRIMARY KEY (`id`))
SELECT email FROM author WHERE id = book.author_id) AS [author_email] FROM book) AS derivedb WHERE RowNumber BETWEEN 11 AND 20';
...
SELECT email FROM author WHERE id = book.author_id) AS author_email FROM book';
