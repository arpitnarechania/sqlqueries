...
create index i_obj_filter on ca_objects(object_id, deleted, 
...
create table ca_user_sort_items( item_id int unsigned not null AUTO_INCREMENT, sort_id int unsigned not null, bundle_name varchar(255) not null, rank smallint unsigned not null default 0, primary key (item_id), constraint fk_ca_user_sort_items_sort_id foreign key (sort_id) references ca_user_sorts (sort_id) on delete restrict on update restrict)
...
SELECT element_id FROM ca_metadata_elements WHERE datatype=? ORDER BY element_id', __CA_ATTRIBUTE_VALUE_URL__);
...
SELECT content FROM ca_media_content_locations WHERE table_num = ? AND row_id = ? AND content LIKE ?
...
SELECT table_num FROM ca_attributes WHERE attribute_id = ?
...
select media from your computer to upload for use as a preview. Use this control to select media from your computer to upload. Use usercontent service User User Name User access User can access MEMEX exhibition web services  User can edit system-wide message in message widget. User can use all statistics viewer plugin functionality. User can use dashboard widget that keeps track of totals for objects and authorities. User can use dashboard widget that lists processing jobs. User can use dashboard widget that lists recent changes in the database. User can use dashboard widget that lists recent user logins. User can use dashboard widget that lists recently created comments. User can use dashboard widget that lists recently created tags. User can use widget that allows usage of an advanced search form in the dashboard. User can use widget that shows recently created items to list new collections in the dashboard. User can use widget that shows recently created items to list new entities i
create index i_collection_filter on ca_collections(collection_id, deleted, 
SELECT * FROM ca_set_items WHERE item_id IN (?)
...
create index i_item_filter on ca_list_items(item_id, deleted, 
...
create table ca_notifications ( notification_id int unsigned not null AUTO_INCREMENT, notification_type tinyint unsigned not null default 0, datetime int unsigned not null, message longtext, is_system		 tinyint unsigned not null default 0, primary key (notification_id), index i_datetime (datetime), index i_notification_type (notification_type))
create table ca_user_sorts( sort_id int unsigned not null AUTO_INCREMENT, table_num tinyint unsigned not null, user_id int unsigned not null, name varchar(255) not null, settings longtext not null, sort_type char(1) null, rank smallint unsigned not null default 0, deleted tinyint unsigned not null default 0, primary key (sort_id), index i_table_num (table_num), index i_user_id (user_id), unique index u_guid (table_num, name), constraint fk_ca_user_sorts_user_id foreign key (user_id) references ca_users (user_id) on delete restrict on update restrict)
create index i_deaccession_type_id on ca_objects(deaccession_type_id);
create table ca_site_templates ( template_id		 int unsigned not null AUTO_INCREMENT, title					varchar(255)		not null, description			text				not null, template				longtext			not null, template_code 		varchar(100)		not null, tags longtext not null, deleted tinyint unsigned not null default 0, primary key (template_id), unique index u_title (title))
...
CREATE INDEX i_name ON ca_representation_annotation_labels(name(128));
create table ca_guids( guid_id int unsigned not null AUTO_INCREMENT, table_num tinyint unsigned not null, row_id int unsigned not null, guid VARCHAR(36) not null, primary key (guid_id), index i_table_num_row_id (table_num, row_id), unique index u_guid (guid))
select media from your computer to upload for use as a preview. Use this control to select media from your computer to upload. Use usercontent service User User %1 was permanently deactivated because the maximum number of consecutive unsuccessful password reset attemps was reached. User Name User access User already has <em>%1</em> User can access MEMEX exhibition web services  User can edit system-wide message in message widget. User can import ULAN data. User can import WorldCat data. User can use all statistics viewer plugin functionality. User can use dashboard widget that keeps track of totals for objects and authorities. User can use dashboard widget that lists processing jobs. User can use dashboard widget that lists recent changes in the database. User can use dashboard widget that lists recent user logins. User can use dashboard widget that lists recently created comments. User can use dashboard widget tha
...
create index i_set_filter on ca_sets(set_id, deleted, 
...
create table ca_data_import_events( event_id int unsigned not null AUTO_INCREMENT, occurred_on int unsigned not null, user_id int unsigned, description text not null, type_code char(50) not null, source text not null, primary key (event_id), constraint fk_ca_data_import_events_user_id foreign key (user_id) references ca_users (user_id) on delete restrict on update restrict)
...
create index i_entity_filter on ca_entities(entity_id, deleted, 
...
SELECT bundle_name FROM ca_user_sort_items WHERE sort_id=? AND rank=?', $this->getPrimaryKey(), $pn_rank);
...
SELECT log_id FROM ca_change_log WHERE log_id = ?
...
create table ca_batch_log_items ( item_id int unsigned not null AUTO_INCREMENT,	batch_id int unsigned not null,	row_id int unsigned not null,	errors longtext null,		primary key (item_id), KEY i_row_id (row_id), INDEX i_batch_row_id (batch_id, row_id), constraint fk_ca_batch_log_items_batch_id foreign key (batch_id) references ca_batch_log (batch_id) on delete restrict on update restrict)
...
SELECT * FROM ca_sets WHERE set_id IN (?)
...
create table ca_set_items (	item_id		int unsigned not null auto_increment,	set_id		int unsigned not null references ca_sets(set_id),	table_num	tinyint unsigned not null,	row_id		int unsigned not null, type_id int unsigned not null,	rank		int unsigned not null default 0,	vars longtext not null,	deleted tinyint unsigned not null default 0,		primary key (item_id),	key i_set_id (set_id, deleted),	key i_type_id (type_id),	key i_row_id (row_id),	key i_table_num (table_num))
...
create table ca_download_log ( log_id		 	int unsigned not null AUTO_INCREMENT, log_datetime 	int unsigned not null, user_id 	int unsigned null, ip_addr			 	varchar(39)			null, table_num 			tinyint unsigned not null, row_id 			int unsigned not null, representation_id int unsigned 	null, download_source		varchar(40)			null, primary key (log_id), constraint fk_ca_download_log_user_id foreign key (user_id) references ca_users (user_id) on delete restrict on update restrict, constraint fk_ca_download_log_representation_id foreign key (representation_id) references ca_object_representations (representation_id) on delete restrict on update restrict, index i_table_num_row_id (table_num, row_id), index i_log_datetime (log_datetime))
...
SELECT * FROM ca_search_indexing_queue WHERE started_on IS NULL ORDER BY entry_id
...
SELECT * FROM {$vs_table} ".((sizeof($va_sql) > 0) ? " WHERE (".join(" AND ", $va_sql).")" : 
...
SELECT * FROM ca_attribute_values WHERE element_id=? ORDER BY value_id', $qr_elements->get('element_id'));
...
SELECT * FROM ca_sql_search_word_index
...
SELECT * FROM ca_guids WHERE guid = ?
...
SELECT access FROM ca_list_items WHERE item_id = ? AND deleted = 0
...
create table ca_site_templates ( template_id		 int unsigned not null AUTO_INCREMENT, title					varchar(255)		not null, description			text				not null, template				longtext			not null, tags longtext not null, deleted tinyint unsigned not null default 0, primary key (template_id), unique index u_title (title))
...
UPDATE ca_search_indexing_queue SET started_on = ?  WHERE entry_id = ?
...
create table ca_site_page_media ( media_id		 	int unsigned not null AUTO_INCREMENT, page_id int unsigned not null references ca_site_pages(page_id), title					varchar(255)		not null, caption			 text				not null, idno varchar(255) not null, idno_sort varchar(255) not null, media 			longblob not null, media_metadata longblob not null, media_content			longtext			not null, md5 varchar(32) not null, mimetype varchar(255) null, original_filename varchar(1024) not null, rank					int unsigned		not null default 0, access tinyint unsigned not null default 0, deleted tinyint unsigned not null default 0, primary key (media_id), key (page_id), key (rank), key (md5), key (idno), key (idno_sort), unique index u_idno (page_id, idno))
...
SELECT bundle_name,rank FROM ca_user_sort_items WHERE sort_id=? ORDER BY rank, item_id', $this->getPrimaryKey());
...
SELECT * FROM ca_set_item_labels WHERE item_id IN (?)
...
SELECT * FROM ca_user_sorts WHERE table_num=? AND deleted=0 ORDER BY rank', $pn_table_num);
...
create table ca_replication_log( entry_id int unsigned not null AUTO_INCREMENT, source_system_guid VARCHAR(36) not null, log_id int unsigned not null, status char(1) not null, vars longtext null, primary key (entry_id), index i_source_log (source_system_guid, log_id))
...
SELECT * FROM ca_user_sorts WHERE deleted=0 ORDER BY table_num, rank');
...
create table ca_search_form_type_restrictions ( restriction_id int unsigned not null AUTO_INCREMENT, table_num tinyint unsigned not null, type_id int unsigned, form_id int unsigned not null, include_subtypes tinyint unsigned not null default 0, settings longtext not null, rank smallint unsigned not null default 0, primary key (restriction_id), index i_form_id				(form_id), index i_type_id				(type_id), constraint fk_ca_search_form_type_restrictions_form_id foreign key (form_id) references ca_search_forms (form_id) on delete restrict on update restrict)
...
create table ca_download_log ( log_id		 	int unsigned not null AUTO_INCREMENT, log_datetime 	int unsigned not null, user_id 	int unsigned null, ip_addr			 	char(15)			null, table_num 			tinyint unsigned not null, row_id 			int unsigned not null, representation_id int unsigned 	null, download_source		varchar(40)			null, primary key (log_id), constraint fk_ca_download_log_user_id foreign key (user_id) references ca_users (user_id) on delete restrict on update restrict, constraint fk_ca_download_log_representation_id foreign key (representation_id) references ca_object_representations (representation_id) on delete restrict on update restrict, index i_table_num_row_id (table_num, row_id), index i_log_datetime (log_datetime))
create index i_name_sort on ca_data_exporter_labels(name_sort(128));
...
SELECT row_id, boost, field_container_id FROM {$ps_dest_table}_acc");
...
create index i_loc_filter on ca_storage_locations(location_id, deleted, 
create table ca_site_pages ( page_id		 	int unsigned not null AUTO_INCREMENT, template_id int unsigned not null references ca_site_templates(template_id), title					varchar(255)		not null, description			text				not null, path 			varchar(255) not null, content				longtext			not null, keywords				text				not null, access tinyint unsigned not null default 0, deleted tinyint unsigned not null default 0, view_count int unsigned not null default 0, primary key (page_id), key (template_id), unique index u_path (path))
create index i_loan_filter on ca_loans(loan_id, deleted, 
...
create index i_lot_filter on ca_object_lots(lot_id, deleted, 
SELECT * FROM {$vs_table} WHERE deleted=1
...
create index i_rep_filter on ca_object_representations(representation_id, deleted, 
...
create table ca_notifications ( notification_id int unsigned not null AUTO_INCREMENT, notification_type tinyint unsigned not null default 0, user_id int unsigned, group_id int unsigned, datetime int unsigned not null, table_num tinyint unsigned, row_id int unsigned, was_read tinyint unsigned not null default 0, message longtext, primary key (notification_id), constraint fk_ca_users_user_id foreign key (user_id) references ca_users (user_id) on delete restrict on update restrict, constraint fk_ca_user_groups_group_id foreign key (group_id) references ca_user_groups (group_id) on delete restrict on update restrict, index i_table_num_row_id (table_num, row_id), index i_datetime (datetime), index i_notification_type (notification_type))
...
create table ca_notification_subjects ( subject_id int unsigned not null auto_increment, notification_id int unsigned not null references ca_notifications(notification_id), was_read tinyint unsigned not null default 0, table_num tinyint unsigned not null, row_id int unsigned not null, primary key (subject_id), index i_notification_id (notification_id), index i_table_num_row_id (table_num, row_id))
...
CREATE index i_index_field_num_container on ca_sql_search_word_index(word_id, table_num, field_table_num, field_num, field_container_id, row_id, 
create index i_type_id on ca_loans_x_movements (type_id);
create index i_set_id on ca_commerce_transactions(set_id);
SELECT attribute_id FROM ca_attribute_values WHERE value_id = ?
...
create index i_occ_filter on ca_occurrences(occurrence_id, deleted, 
...
select media from your computer to upload for use as a preview. Use usercontent service User User Name User access User activity for %1 User already has <em>%1</em> User can import ULAN data. User class User comments User could not be found. User defined sort order User generated content User groups User holds: User id User id %1 does not exist User interface editor User interface screen editor User interface theme User interfaces User interfaces for %1 User logins User name User name %1 does not exist User name to authenticate with on remote system. User name to reset password for. User profile User tags User who performed data import User who performed data import. User who performed event User-provided rating for item. Users Using interface Utah Uzbekistan Validate all records against metadata dictionary Validate all records against rules in metadata dictionary. Validation format Value Value %1 is not in list %2 Value %3 for %1 [%2] is invalid Value <em>%1</em> is already used and d
...
SELECT value_id FROM ca_attribute_values WHERE element_id=? AND value_longtext1=?', $pn_element_id, $ps_val);
...
SELECT * FROM ca_attributes where attribute_id=?', $qr_vals->get('attribute_id'));
...
SELECT * FROM ca_user_sort_items WHERE sort_id=? ORDER BY rank, item_id', $this->getPrimaryKey());
create table ca_sql_search_ngrams ( word_id int(10) unsigned not null, ngram char(4) not null, seq tinyint(3) unsigned not null, primary key (word_id,seq))
...
create table ca_site_page_media ( media_id		 	int unsigned not null AUTO_INCREMENT, page_id int unsigned not null references ca_site_pages(page_id), title					varchar(255)		not null, idno varchar(255) not null, idno_sort varchar(255) not null, caption			 text				not null, media 			longblob not null, media_metadata longblob not null, media_content			longtext			not null, md5 varchar(32) not null, mimetype varchar(255) null, original_filename varchar(1024) not null, rank					int unsigned		not null default 0, access tinyint unsigned not null default 0, deleted tinyint unsigned not null default 0, primary key (media_id), key (page_id), key (rank), key (md5), key (idno), key (idno_sort), unique index u_idno (page_id, idno))
...
create table ca_site_pages ( page_id		 	int unsigned not null AUTO_INCREMENT, template_id int unsigned not null references ca_site_templates(template_id), title					varchar(255)		not null, description			text				not null, path 			varchar(255) not null, content				longtext			not null, keywords				text				not null, access tinyint unsigned not null default 0, deleted tinyint unsigned not null default 0, view_count int unsigned not null default 0, primary key (page_id), key (template_id), key (path))
create index i_name on ca_storage_location_labels(name(128));
...
SELECT row_id, item_id, rank FROM ca_set_items WHERE set_id = ? AND deleted = 0 ORDER BY rank
create index i_name on ca_list_item_labels(	item_id,	name_plural(128));
...
create index i_movement_filter on ca_movements(movement_id, deleted, 
