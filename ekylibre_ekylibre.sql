UPDATE product_natures SET variety=citrullus_lanatus_gigérine  WHERE variety=citrullus_lanatus_gigerine
...
CREATE TABLE financial_year_exchanges ( id integer NOT NULL, financial_year_id integer NOT NULL, started_on date NOT NULL, stopped_on date NOT NULL, closed_at timestamp without time zone, public_token character varying, public_token_expired_at timestamp without time zone, import_file_file_name character varying, import_file_content_type character varying, import_file_file_size integer, import_file_updated_at timestamp without time zone, created_at timestamp without time zone NOT NULL, updated_at timestamp without time zone NOT NULL, creator_id integer, updater_id integer, lock_version integer DEFAULT 0 NOT NULL)
...
CREATE INDEX index_tax_declaration_item_parts_on_created_at ON tax_declaration_item_parts 
...
CREATE INDEX index_fixed_assets_on_scrapped_journal_entry_id ON fixed_assets 
...
CREATE INDEX index_outgoing_payment_lists_on_mode_id ON outgoing_payment_lists 
...
CREATE INDEX index_products_on_activity_production_id ON products 
...
CREATE INDEX index_payslips_on_updated_at ON payslips 
...
CREATE INDEX index_products_on_originator_id ON products 
...
CREATE INDEX index_regularizations_on_updater_id ON regularizations 
...
UPDATE interventions SET procedure_name = animal_veterinary_treatment  WHERE procedure_name = animal_antibiotic_treatment
...
CREATE INDEX index_debt_transfers_on_updater_id ON debt_transfers 
CREATE INDEX index_net_services_on_creator_id ON net_services 
CREATE INDEX index_inspections_on_updater_id ON inspections 
CREATE INDEX index_tax_declaration_item_parts_on_journal_entry_item_id ON tax_declaration_item_parts 
...
UPDATE map_layers SET nature=background  WHERE nature=overlay
...
SELECT reference_name FROM map_layers WHERE nature='overlay'
...
CREATE INDEX index_payslips_on_started_on ON payslips 
...
SELECT charge_account_id FROM product_nature_variants AS v JOIN product_nature_categories AS c ON (v.category_id = c.id) WHERE id = #{variant_id}
SELECT count(*) FROM purchase_natures WHERE nature = 'payslip'
...
CREATE INDEX index_debt_transfers_on_creator_id ON debt_transfers 
...
CREATE INDEX index_payslips_on_journal_entry_id ON payslips 
...
UPDATE product_natures SET derivative_of=citrullus_lanatus_gigérine  WHERE derivative_of=citrullus_lanatus_gigerine
...
UPDATE activities SET cultivation_variety=citrullus_lanatus_gigérine  WHERE cultivation_variety=citrullus_lanatus_gigerine
...
CREATE TABLE debt_transfers ( id integer NOT NULL, affair_id integer NOT NULL, debt_transfer_affair_id integer NOT NULL, amount numeric(19,4) DEFAULT 0.0, number character varying, nature character varying NOT NULL, currency character varying NOT NULL, journal_entry_id integer, accounted_at timestamp without time zone, created_at timestamp without time zone NOT NULL, updated_at timestamp without time zone NOT NULL, creator_id integer, updater_id integer, lock_version integer DEFAULT 0 NOT NULL)
...
UPDATE manure_management_plan_zones SET cultivation_variety=citrullus_lanatus_gigerine  WHERE cultivation_variety=citrullus_lanatus_gigérine
...
CREATE INDEX index_payslips_on_creator_id ON payslips 
...
UPDATE product_natures SET derivative_of=citrullus_lanatus_gigerine  WHERE derivative_of=citrullus_lanatus_gigérine
...
UPDATE products SET derivative_of=citrullus_lanatus_gigerine  WHERE derivative_of=citrullus_lanatus_gigérine
...
UPDATE product_nature_variants SET derivative_of=citrullus_lanatus_gigerine  WHERE derivative_of=citrullus_lanatus_gigérine
...
CREATE INDEX index_map_layers_on_updater_id ON map_layers 
UPDATE products SET variety=pisum_sativum_navarro  WHERE variety=pisum_vernum_navarro
CREATE INDEX index_financial_year_exchanges_on_financial_year_id ON financial_year_exchanges 
...
CREATE TABLE tax_declaration_item_parts ( id integer NOT NULL, tax_declaration_item_id integer NOT NULL, journal_entry_item_id integer NOT NULL, account_id integer NOT NULL, tax_amount numeric(19,4) NOT NULL, pretax_amount numeric(19,4) NOT NULL, total_tax_amount numeric(19,4) NOT NULL, total_pretax_amount numeric(19,4) NOT NULL, direction character varying NOT NULL, created_at timestamp without time zone NOT NULL, updated_at timestamp without time zone NOT NULL, creator_id integer, updater_id integer, lock_version integer DEFAULT 0 NOT NULL)
...
CREATE TABLE outgoing_payment_lists ( id integer NOT NULL, number character varying, created_at timestamp without time zone, updated_at timestamp without time zone, creator_id integer, updater_id integer, lock_version integer DEFAULT 0 NOT NULL, mode_id integer NOT NULL, cached_payment_count integer, cached_total_sum numeric)
...
CREATE INDEX index_regularizations_on_created_at ON regularizations 
...
CREATE INDEX index_payslip_natures_on_creator_id ON payslip_natures 
...
CREATE INDEX index_journal_entry_items_on_variant_id ON journal_entry_items 
...
SELECT id FROM product_nature_variants WHERE variety = 'worker' LIMIT 1
...
CREATE INDEX index_tax_declaration_item_parts_on_updater_id ON tax_declaration_item_parts 
...
CREATE INDEX index_debt_transfers_on_created_at ON debt_transfers 
...
UPDATE interventions SET working_duration = s WHERE working_duration IS NULL
...
SELECT id FROM product_natures WHERE variety = 'worker' ORDER BY id LIMIT 1
...
CREATE TABLE payslips ( id integer NOT NULL, number character varying NOT NULL, nature_id integer NOT NULL, employee_id integer, account_id integer, started_on date NOT NULL, stopped_on date NOT NULL, emitted_on date, state character varying NOT NULL, amount numeric(19,4) NOT NULL, currency character varying NOT NULL, accounted_at timestamp without time zone, journal_entry_id integer, affair_id integer, custom_fields jsonb, created_at timestamp without time zone NOT NULL, updated_at timestamp without time zone NOT NULL, creator_id integer, updater_id integer, lock_version integer DEFAULT 0 NOT NULL)
...
UPDATE manure_management_plan_zones SET cultivation_variety=citrullus_lanatus_gigérine  WHERE cultivation_variety=citrullus_lanatus_gigerine
...
UPDATE product_nature_variants SET variety=citrullus_lanatus_gigerine  WHERE variety=citrullus_lanatus_gigérine
...
CREATE INDEX index_map_layers_on_updated_at ON map_layers 
...
UPDATE affairs SET type = PayslipAffair  WHERE id IN (SELECT affair_id FROM payslips)
...
UPDATE outgoing_payments SET type = PayslipPayment  WHERE affair_id IN (SELECT id FROM affairs WHERE type = PayslipAffair)
CREATE INDEX index_financial_years_on_accountant_id ON financial_years 
...
CREATE INDEX index_debt_transfers_on_debt_transfer_affair_id ON debt_transfers 
...
CREATE INDEX index_payslip_natures_on_updated_at ON payslip_natures 
...
CREATE INDEX index_regularizations_on_journal_entry_id ON regularizations 
...
CREATE INDEX index_payslips_on_account_id ON payslips 
...
CREATE INDEX index_purchase_items_on_fixed_asset_id ON purchase_items 
...
CREATE INDEX index_map_layers_on_name ON map_layers 
...
UPDATE activities SET support_variety=citrullus_lanatus_gigérine  WHERE support_variety=citrullus_lanatus_gigerine
...
CREATE INDEX index_fixed_assets_on_asset_account_id ON fixed_assets 
...
CREATE INDEX index_fixed_assets_on_sold_journal_entry_id ON fixed_assets 
...
CREATE INDEX index_map_layers_on_creator_id ON map_layers 
CREATE INDEX index_sensors_on_model_euid ON sensors 
CREATE INDEX index_loan_repayments_on_created_at ON loan_repayments 
CREATE INDEX index_regularizations_on_affair_id ON regularizations 
...
SELECT id FROM accounts WHERE usages = 'staff_expenses' LIMIT 1
...
CREATE INDEX index_tax_declaration_item_parts_on_creator_id ON tax_declaration_item_parts 
...
CREATE INDEX index_map_layers_on_created_at ON map_layers 
...
CREATE INDEX index_journals_on_accountant_id ON journals 
...
CREATE INDEX index_payslips_on_affair_id ON payslips 
...
UPDATE activities SET support_variety=citrullus_lanatus_gigerine  WHERE support_variety=citrullus_lanatus_gigérine
...
UPDATE products SET variety=citrullus_lanatus_gigerine  WHERE variety=citrullus_lanatus_gigérine
...
CREATE TABLE payslip_natures ( id integer NOT NULL, name character varying NOT NULL, currency character varying NOT NULL, active boolean DEFAULT false NOT NULL, by_default boolean DEFAULT false NOT NULL, with_accounting boolean DEFAULT false NOT NULL, journal_id integer NOT NULL, account_id integer, created_at timestamp without time zone NOT NULL, updated_at timestamp without time zone NOT NULL, creator_id integer, updater_id integer, lock_version integer DEFAULT 0 NOT NULL)
...
UPDATE interventions SET procedure_name = animal_antibiotic_treatment  WHERE procedure_name = animal_veterinary_treatment
...
CREATE INDEX index_debt_transfers_on_affair_id ON debt_transfers 
...
CREATE INDEX index_payslip_natures_on_journal_id ON payslip_natures 
...
UPDATE product_natures SET population_counting = unitary  WHERE reference_name IN (#{PRODUCT_NATURES.map { |v| quote(v) }.join(, )})
...
UPDATE interventions SET procedure_name = animal_group_changing, actions = animal_group_changing  WHERE procedure_name = animal_changing
...
CREATE INDEX index_payslips_on_number ON payslips 
...
CREATE INDEX index_payslip_natures_on_account_id ON payslip_natures 
...
SELECT tax_id FROM product_nature_variants AS v JOIN product_nature_categories AS c ON (v.category_id = c.id) WHERE id = #{variant_id}
...
CREATE INDEX index_payslip_natures_on_updater_id ON payslip_natures 
...
CREATE INDEX index_tax_declaration_item_parts_on_tax_declaration_item_id ON tax_declaration_item_parts 
...
CREATE INDEX index_financial_year_exchanges_on_creator_id ON financial_year_exchanges 
...
CREATE INDEX index_crumbs_on_intervention_participation_id ON crumbs 
...
SELECT id FROM product_nature_categories WHERE reference_name = 'worker' ORDER BY id LIMIT 1
...
CREATE INDEX index_cashes_on_suspense_account_id ON cashes 
...
CREATE INDEX index_payslips_on_stopped_on ON payslips 
...
CREATE INDEX index_tax_declaration_item_parts_on_account_id ON tax_declaration_item_parts 
...
CREATE INDEX index_financial_year_exchanges_on_created_at ON financial_year_exchanges 
...
UPDATE interventions SET started_at = s WHERE started_at IS NULL
...
UPDATE product_nature_variants SET derivative_of=citrullus_lanatus_gigérine  WHERE derivative_of=citrullus_lanatus_gigerine
...
UPDATE interventions SET procedure_name = animal_housing_disinfection  WHERE procedure_name = housing_soil_disinfection
...
CREATE INDEX index_financial_year_exchanges_on_updated_at ON financial_year_exchanges 
...
UPDATE journal_entry_items SET letter = letter  WHERE letter IS NOT NULL
UPDATE product_nature_variants SET derivative_of=pisum_vernum_kayanne  WHERE derivative_of=pisum_sativum_kayanne
CREATE INDEX index_tax_declaration_item_parts_on_updated_at ON tax_declaration_item_parts 
...
CREATE INDEX index_debt_transfers_on_updated_at ON debt_transfers 
...
CREATE INDEX index_journal_entries_on_financial_year_exchange_id ON journal_entries 
...
UPDATE products SET variety=citrullus_lanatus_gigérine  WHERE variety=citrullus_lanatus_gigerine
...
UPDATE interventions SET procedure_name = herd_feeding  WHERE procedure_name = silage_unload
UPDATE interventions SET whole_duration = EXTRACT WHERE whole_duration IS NULL
...
CREATE INDEX index_financial_year_exchanges_on_updater_id ON financial_year_exchanges 
...
CREATE INDEX index_payslips_on_employee_id ON payslips 
...
UPDATE interventions SET stopped_at = started_at + 1 WHERE stopped_at IS NULL
...
CREATE INDEX index_regularizations_on_creator_id ON regularizations 
...
UPDATE interventions SET procedure_name = housing_soil_disinfection  WHERE procedure_name = animal_housing_disinfection
CREATE INDEX index_cash_sessions_on_cash_id ON cash_sessions 
CREATE INDEX index_cashes_on_main_account_id ON cashes 
...
UPDATE payslip_natures SET by_default = TRUE  WHERE id IN (SELECT id FROM payslip_natures ORDER BY id LIMIT 1)
...
UPDATE product_nature_variants SET variety=citrullus_lanatus_gigérine  WHERE variety=citrullus_lanatus_gigerine
...
UPDATE intervention_parameters SET group_id = groups WHERE type = InterventionGroupParameter
CREATE INDEX index_regularizations_on_updated_at ON regularizations 
...
CREATE INDEX index_payslips_on_updater_id ON payslips 
...
CREATE INDEX index_bank_statements_on_journal_entry_id ON bank_statements 
...
UPDATE products SET derivative_of=citrullus_lanatus_gigérine  WHERE derivative_of=citrullus_lanatus_gigerine
CREATE INDEX index_parcel_items_on_updater_id ON parcel_items 
UPDATE product_natures SET variety=citrullus_lanatus_gigerine  WHERE variety=citrullus_lanatus_gigérine
...
CREATE INDEX index_tax_declaration_item_parts_on_direction ON tax_declaration_item_parts 
...
CREATE INDEX index_journal_entry_items_on_tax_declaration_mode ON journal_entry_items 
...
CREATE INDEX index_fixed_assets_on_product_id ON fixed_assets 
...
UPDATE activities SET cultivation_variety=citrullus_lanatus_gigerine  WHERE cultivation_variety=citrullus_lanatus_gigérine
CREATE INDEX index_integrations_on_creator_id ON integrations 
CREATE INDEX index_payslips_on_created_at ON payslips 
...
UPDATE interventions SET procedure_name = silage_unload  WHERE procedure_name = herd_feeding
CREATE INDEX index_sales_on_affair_id ON sales 
CREATE INDEX index_journal_entry_items_on_tax_id ON journal_entry_items 
CREATE INDEX index_purchase_items_on_depreciable_product_id ON purchase_items 
...
CREATE INDEX index_fixed_assets_on_journal_entry_id ON fixed_assets 
...
CREATE INDEX index_payslips_on_nature_id ON payslips 
...
CREATE TABLE regularizations ( id integer NOT NULL, affair_id integer NOT NULL, journal_entry_id integer NOT NULL, currency character varying NOT NULL, created_at timestamp without time zone NOT NULL, updated_at timestamp without time zone NOT NULL, creator_id integer, updater_id integer, lock_version integer DEFAULT 0 NOT NULL)
...
SELECT affair_id FROM letterable_multi_thirds WHERE count != 1);
