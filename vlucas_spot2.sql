...
SELECT * FROM test_legacy WHERE test_legacy." . self::$legacyTable->getNumberFieldColumnName() . " = ? GROUP BY TRIM(test_legacy." . self::$legacyTable->getNameFieldColumnName() . ")
...
SELECT * FROM test_posts WHERE test_posts.status = ? GROUP BY TRIM(test_posts.body)
