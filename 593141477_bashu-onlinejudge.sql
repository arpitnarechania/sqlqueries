...
select user_id from users where TIMESTAMPADD(DAY,-1,NOW())<accesstime order by user_id
...
select defunct,has_tex,compare_way from problem where problem_id=$prob_id
...
select problem_id from problem_category where category='".mysql_real_escape_string($category)."' )";
select max(problem_id) from problem');
select user_id,rightstr from privilege order by user_id
CREATE TABLE `problem_category` ( `problem_id` int(11) NOT NULL, `category` varchar(20) NOT NULL, PRIMARY KEY (`category`,`problem_id`))
...
SELECT experience from level_experience where level=get_problem_level(%d))as t set submit=submit+1,solved=solved+%d,score=score+%d,users.experience=users.experience+IFNULL(t.experience,0)*%d where user_id='%s'", data->problem, is_first_solved, delta, is_first_solved, data->user.c_str());
...
select category from problem_category where problem_id=$prob_id
...
select compare_way from problem where problem_id=$row[6]
